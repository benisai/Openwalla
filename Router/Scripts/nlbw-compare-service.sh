...
# Service name
SERVICE_NAME="nlbw-compare-service.sh"
...
enable() {
    echo "Enabling nlbw-compare.sh service..."
    # Create symlink to startup directory
    ln -s /etc/init.d/nlbw-compare-service.sh /etc/rc.d/S99nlbw-compare-service.sh
    echo "Service enabled."
}

# Disable the service (remove symlink)
disable() {
    echo "Disabling nlbw-compare.sh service..."
    rm -f /etc/rc.d/S99nlbw-compare-service.sh
...
status() {
    ...
    if [ -n "$pids" ]; then
        echo "nlbw-compare.sh is running with PID: $pids"
    else
        echo "nlbw-compare.sh is not running."
    fi
}
