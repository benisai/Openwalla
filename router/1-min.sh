...
#nlbw -c csv -g ip,mac -o ip | tr -d '"' | tail -n +2 | awk '{print $1 "," $2 "," $3 "," $4 "," $5 "," $6 "," $7}' > /tmp/nlbwmon.out
#nlbw -c csv -g ip,mac -o ip | tr -d '"'  | awk '{print $1 "," $2 "," $3 "," $4 "," $5 "," $6 "," $7}' > /tmp/nlbwmon.out

ln -s /tmp/nlbwmon.out /www/nlbw.html
...
ln -s /tmp/vnstat.out /www/vnstat.txt
